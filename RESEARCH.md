...
## v5 Research Validation: Ablation Study + Experiments

`evaluation/research_experiments.py` answers the four outstanding
research-validation items from the v5 review, all on the reproducible
synthetic dataset (no external download needed):
...
readable table for each experiment.

**1. Ablation study** — single causal pass over the synthetic dataset,
reconstructing each variant's score post-hoc from `RiskResult.raw_components`
(no re-running detection, no leakage): signature-only → ML-only →
signature+ML → full four-signal fusion → +rule/MITRE/asset/trust boosts →
full pipeline (+ history). Shows each layer's marginal effect on precision/
recall/F1/FPR.

**2. Adaptive learning** — genuine held-out split (train/test, no leakage):
`AdaptiveWeightLearner` is trained on the first half using ground truth as
simulated analyst feedback (mirrors `main.py`'s real feedback wiring), then
evaluated against the FIXED default weights on the untouched second half.
Both passes score identical detections, so any metric delta is attributable
to the weight change alone.

**3. Correlation false-positive reduction** — synthetic multi-stage
campaigns (genuine attacker IPs firing a real Port Scan → SSH Bruteforce →
Anomalous Behaviour chain) mixed with isolated single-event benign noise.
Compares raw per-event alerting against `CorrelationEngine`-confirmed
incidents on the same event stream, reporting the resulting false-positive
reduction and precision gain.

**4. Historical risk before/after** — deliberately weak per-event signal
(MEDIUM, never HIGH on its own) replayed for a pool of repeat-offender IPs
vs. a pool of one-off IPs. A causal, no-lookahead recidivism counter drives
`history_boost` exactly the way `RiskEngine` folds it in; comparing the
recorded score against the same event *without* its `history_boost` term
isolates recidivism's effect on repeat-offender recall at the HIGH bar,
while confirming the one-off control population is unaffected.
