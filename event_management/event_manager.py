...
from risk_engine.risk_scorer import RiskResult
...
class SecurityEvent:
    ...
    def from_detection(
        ...
        return cls(
            ...
            signature_rule=(
                det.signature_result.rule_id
                if det.signature_result
                else None
            ),
            anomaly_label=(
                det.anomaly_result.label
                if det.anomaly_result
                else "Normal"
            ),
            anomaly_score=det.anomaly_score,
            risk_score=risk.overall,
            risk_components=risk.to_dict().get(
                "components",
                {}
            )
        )
...
class EventManager:
    ...
    def store(
        ...
        ev = SecurityEvent.from_detection(
            det,
            risk,
            self._next_id()
        )
        ...
        if enrichment:
            ...
            ev.country = enrichment.get(
                "country",
                ev.country
            )

            ev.organization = enrichment.get(
                "organization",
                ev.organization
            )

            ev.dns_name = enrichment.get(
                "dns_name",
                ev.dns_name
            )

            ev.is_private = enrichment.get(
                "is_private",
                ev.is_private
            )
        ...
        if self._mongo:
            ...
            try:
                ...
                # insert_one mutates d with ObjectId
                # — strip it
    ...
    def recent_events(self, limit=50):
        return self._mem.recent(limit)

    def total_count(self):
        return self._mem.count()

    def severity_counts(self):
        return self._mem.sev_counts()

    def attack_type_counts(self):
        return self._mem.attack_counts()

    def protocol_counts(self):
        return self._mem.proto_counts()

    def risk_trend(self, n=30):
        return self._mem.risk_trend(n)
