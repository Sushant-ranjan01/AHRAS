...
class SecurityEvent:
    ...
    def from_detection(cls, det:DetectionResult, risk:RiskScore, eid:str):
        ...
        return cls(event_id=eid,flow_id=det.flow_id,
                   ...
                   # `risk.overall` is the 0-1 rescaling of risk_score_100 (see risk_scorer.py) --
                   # every consumer of this event's risk_score (dashboard live feed, event table,
                   # risk trend chart) renders it on a 0-10 scale ("Risk=X/10", y-axis max:10,
                   # color thresholds at 7/4/2). Feeding it `overall` (max 1.0) meant risk_score
                   # could never exceed 1, so every event rendered as low/green regardless of
                   # actual severity. risk_score_100/10 is the correctly-scaled 0-10 value.
                   risk_score=risk.risk_score_100/10.0,risk_components=risk.to_dict().get("components",{}))
