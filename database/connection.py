...
def _create_indexes():
    ...
    try:
        ...
        _db.audit_logs.create_index([("actor", ASCENDING)])
        _db.audit_logs.create_index([("action", ASCENDING)])
        _db.audit_logs.create_index([("ip", ASCENDING)])
        _db.audit_logs.create_index([("timestamp", ASCENDING)], expireAfterSeconds=365*86400, name="audit_ttl")
