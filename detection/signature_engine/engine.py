...
class SignatureEngine:
    ...
    def _flood(self, flow):
        # BUG FIX (matches the same artifact already documented/fixed in
        # risk_engine/risk_scorer.py's pps bonus): packets_per_second is
        # packet_count / duration, and duration for a short-lived flow
        # (flow_generator.py finalise() floors it at 0.001s) can be tiny.
        # A completely ordinary flow -- 2 DNS packets 8ms apart, a handful
        # of QUIC/UDP video-call packets in one burst, a browser's rapid
        # ACKs -- divides out to a "PPS" of thousands, blowing past
        # FLOOD_THRESHOLD with nothing resembling a real flood underneath
        # it. Confirmed live: this was firing "Traffic Flood"/"UDP Flood"/
        # "High Packet Rate" on ordinary browsing/video-call traffic to
        # Google/Cloudflare/Microsoft/Amazon. A genuine flood has BOTH a
        # high computed rate AND an actual volume of packets; requiring a
        # minimum packet_count filters out the short-flow division
        # artifact while still catching real floods (which have far more
        # than a couple dozen packets).
        if flow.packets_per_second >= config.FLOOD_THRESHOLD and flow.packet_count >= 20:
            return SignatureResult("Traffic Flood", 1.0, "CRITICAL", "SIG-002", f"PPS={flow.packets_per_second:.0f}")

    def _udp_flood(self, flow):
        if (flow.protocol=="UDP" and flow.packets_per_second > config.FLOOD_THRESHOLD*0.3
                and flow.unique_ports<=3 and flow.packet_count >= 20):
    ...
    def _high_rate(self, flow):
        ...
        if flow.packets_per_second > t and flow.packet_count >= 20:
