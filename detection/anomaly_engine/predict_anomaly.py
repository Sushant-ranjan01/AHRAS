...
@dataclass
class AnomalyResult:
    # NOTE on `confidence`: this is NOT a calibrated statistical probability
    # of "this is an attack". It's `abs(normalised_score - 0.5) * 2` -- i.e.
    # how far the normalised anomaly score sits from the ambiguous midpoint
    # of the calibration window, on a 0-1 scale. A confidence of 1.0 means
    # "this score sits at the extreme edge of the calibrated range" and is
    # produced identically whether that edge is the very-normal end or the
    # very-anomalous end. Treat it as "how decisively the model placed this
    # point", not as P(attack). There is no attack-labeled calibration set
    # backing this number (see train_model.py) -- producing a true
    # probability would need one.
