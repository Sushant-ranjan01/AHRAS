...
def train(output_path=None, n_samples=None, baseline_csv=None):
    ...
    # -------------------------------------------------------------------
    # Calibration: stretch decision_function()'s observed spread on
    # HELD-OUT benign data to [0, 1] instead of assuming a fixed window.
    #
    ...
    #
    # v2 (25-08-26): X is real labeled-benign flows (_real_benign), so the
    # percentile window reflects an actual traffic distribution. But it was
    # still calibrated on the SAME rows the model was fit on -- decision_function
    # is typically wider/more confident on training rows than on genuinely
    # unseen benign traffic, so this window can still be too narrow in
    # production and clip real unseen benign flows toward the rails.
    #
    # v3 (this version): fit the pipeline on an 80% split and calibrate the
    # percentile window on the other 20%, held out from fitting entirely.
    # Falls back to fitting-set calibration only when there isn't enough
    # data for a meaningful split (logged explicitly, not silent).
    # -------------------------------------------------------------------
    if len(X) >= 100:
        X_fit, X_calib = train_test_split(X, test_size=0.2, random_state=42)
    else:
        logger.warning(
            "Only %d benign rows -- too few for a held-out calibration "
            "split; calibrating on the fitting data itself (v2 behaviour).",
            len(X),
        )
        X_fit, X_calib = X, X
