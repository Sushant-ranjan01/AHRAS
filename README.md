...
**Default login:** `admin / Admin@12345!` (also `analyst / Analyst@12345!`, `manager / Manager@12345!` —
...
3. **Change the seeded default passwords** (`admin/Admin@12345!`, `analyst/Analyst@12345!`,
   `manager/Manager@12345!`) before this is reachable by anyone but you.
...
AHRAS now includes a dedicated quality gate covering authentication, RBAC, refresh-token lifecycle, API route protection, static analysis, dependency auditing, coverage, and opt-in risk-engine performance regression tests. See `TESTING_PHASE3.md` for the commands and workflow.


## Phase 5 — SOC Analyst Command Center

A dedicated SOC Console now provides alert triage, investigation drill-down, related-event timelines, MITRE and threat-intelligence context, linked cases, case creation from alerts, role-aware response actions, and authenticated CSV alert export. See `PHASE_5_SOC_CONSOLE.md`.
