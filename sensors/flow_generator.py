...
class FlowRecord:
    ...
    def finalise(self):
        ...
        # BUG FIX: packets_per_second/bytes_per_second must not be divided
        # by self.duration directly when duration is sub-second. A handful
        # of packets a few milliseconds apart -- an ordinary DNS
        # query+response, a couple of QUIC/UDP video-call packets in one
        # burst, a browser's rapid TCP ACKs -- divides out to a "rate" of
        # thousands-to-millions of packets/sec: nothing resembling a real
        # flood, but indistinguishable from one to every downstream
        # consumer (signature engine's flood rules, and the trained
        # anomaly model's packets_per_second feature, which is clamped to
        # [0,1000] and saturates to its max/most-anomalous value for any
        # of these ordinary short bursts). Confirmed live: this was the
        # root cause of "Traffic Flood"/"UDP Flood"/"High Packet Rate"/
        # "Anomalous Behaviour" firing on completely ordinary browsing and
        # video-call traffic to Google/Cloudflare/Microsoft/Amazon.
        # Flooring the rate divisor at 1 real second removes the
        # sub-second division artifact; genuine floods (hundreds+ packets
        # before the flow closes) still compute correctly high.
        rate_divisor = max(self.duration, 1.0)
        self.packets_per_second = self.packet_count / rate_divisor
        self.bytes_per_second   = self.byte_count   / rate_divisor
