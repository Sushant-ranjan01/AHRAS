...
_TRUSTED = ["google", "amazon", "cloudflare", "microsoft", "facebook", "meta platforms", "akamai", "fastly", "apple", "github"]
...
def _trust(ip, skip_dns=False, organization=None):
    if not isinstance(ip, str) or not ip:
        return 0.1, "unknown"
    if _is_private(ip):
        return 0.95, "private-network"
    # Prefer the org name enrichment already resolved (GeoIP/ASN lookup --
    # the same value the dashboard shows in the org column) over a second,
    # independent reverse-DNS guess. Reverse DNS is unreliable for exactly
    # the traffic we most want to trust: most CDN/cloud edge IPs (Cloudflare,
    # Azure, many Google/Meta edge ranges) simply have no PTR record at all,
    # and some that do resolve to a CDN-specific domain (e.g. "fbcdn.net",
    # "1e100.net") that doesn't contain the provider's name as a substring.
    # A resolved org name is direct evidence and doesn't have that problem.
    if organization and organization not in ("Unknown", "Local", "", None):
        org_l = organization.lower()
        matched = next((d for d in _TRUSTED if d in org_l), None)
        if matched:
            result = (0.8, matched)
            _TRUST_CACHE[ip] = result
            return result
        # We already have a concrete org name and it's not a trusted one --
        # don't bother with a second rDNS lookup for a verdict we already have.
        result = (0.3, organization)
        _TRUST_CACHE[ip] = result
        return result
...
class RiskEngine:
    ...
    def evaluate(self, det) -> RiskResult:
        if hasattr(det, "to_dict"):
            ...
            organization = d.get("organization", getattr(det, "organization", None))
        else:
        ...
        # density used to gate on `attack_type != "Normal"` -- i.e. it only
        # accumulated once some upstream engine had ALREADY decided this was
        # an attack, then fed that same decision back in as a risk signal
        # (attack_type -> density -> risk). That's circular reinforcement.
        # Gate on the continuous signature/anomaly evidence itself instead,
        # so density reflects "how often has this IP shown suspicious
        # signal recently" independent of any single engine's final label.
        suspicious = (signature_signal >= 0.20) or (anomaly_signal >= 0.50)
        density = self._density(src_ip, suspicious, now=event_time)
        ...
        trust, domain = _trust(src_ip, skip_dns=self._skip_dns, organization=organization)
        ...
        return RiskResult(
            src_ip=src_ip,
            domain=domain,
            # overall used to be risk/10.0, which saturates to 1.0 for any
            # risk >= 10 -- i.e. LOW, MEDIUM, HIGH and CRITICAL all display
            # as an identical "overall": 1.0 (see risk_score_100, which is
            # the correctly-scaled 0-100 field and does NOT saturate).
            # risk/100.0 keeps overall as a genuine 0-1 rescaling of the
            # same 0-100 risk value with no precision loss.
            overall=min(risk / 100.0, 1.0),
    ...
    def _density(self, ip, suspicious, now=None):
        if not suspicious:
            return 0.0
        now = _safe(now, time.time())
