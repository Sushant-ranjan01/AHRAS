...
Risk Formula:
  R = (α·Sig + β·Anomaly + γ·Density + δ·(Drift+Rate)) * 100
    * signal-strength multiplier
    ...
    + MITRE severity boost + Asset criticality boost + Historical-recidivism boost
    capped to [0, 100]

Every term above is recorded verbatim in RiskResult.raw_components so that
risk_explainer.RiskExplainer can reconstruct the score EXACTLY (not
re-derive it from a different formula) — see risk_explainer/explainer.py
for the "faithful-by-construction" XAI design this enables.

── Architecture decision: which modules feed the per-event score ───────────
Only FOUR sources are treated as formal risk-score INPUTS:
  1) the four weighted base signals (signature / anomaly / density / drift+rate)
  2) rule-based / MITRE / asset-criticality boosts (deterministic, per-event)
  3) trust (reverse-DNS reputation) discount
  4) historical recidivism boost (historical_risk.HistoricalRiskEngine.get_boost)
...
contextual modules instead, for two concrete reasons:
  - UBA operates on a different entity/event stream (authenticated username
    logins) than RiskEngine (network flow src_ip/dst_ip detections). There
    is no reliable 1:1 join key between "this flow" and "this login" in the
    current data model, so any wiring would be a guess, not a signal.
  - The threat graph and the attack forecaster are BUILT FROM already-scored
    ...
    creates a circular dependency and risks a feedback loop (rising
    forecast → higher score → higher forecast → ...). Historical risk avoids
    this because it is a simple counter of past occurrences, not a model
    trained on past scores.
...
class RiskResult:
    def __init__(self,**k):
        ...
        self.raw_components=k.get("raw_components",{})  # for adaptive_learning feedback + RiskExplainer
    ...
    def to_dict(self):
        return {
            ...
            "asset_boost":round(self.asset_boost,1),"history_boost":round(self.history_boost,1),
...
class RiskEngine:
    ...
    def __init__(self, skip_dns=False):
        ...
        logger.info("RiskEngine ready (v5 multi-signal fusion)" + (" [skip_dns]" if skip_dns else ""))
    ...
    def evaluate(self, det) -> RiskResult:
        if hasattr(det,"to_dict"):
            ...
            history_boost=_safe(getattr(det,"history_boost",d.get("history_boost",0.0)))
        else:
        ...
        history_boost=max(0.0,min(45.0,history_boost))  # matches HistoricalRiskEngine's own cap (30+15)
        ...
        base_risk=(alpha*S+beta*A+gamma*T+delta*(B+R))*100
        risk=base_risk
        ...
        rule_boosts=[]
        if unique_ports>10:  rule_boosts.append(("unique_ports>10",30.0))
        if unique_ports>30:  rule_boosts.append(("unique_ports>30",50.0))
        if syn_count>50:     rule_boosts.append(("syn_count>50",40.0))
        if rate>20:          rule_boosts.append(("rate>20",30.0))
        if pps>500 and packet_count>=20: rule_boosts.append(("pps>500_and_packet_count>=20",20.0))
            ...
            if strength < 2:
                return RiskResult(
                    ...
                    raw_components={
                        ...
                        "base_risk":round(base_risk,2),"strength":strength,"strength_multiplier":1.0,
                        "risk_after_strength":round(base_risk,2),"rule_boosts":[],"rule_boost_total":0.0,
                        "risk_after_rules":round(base_risk,2),
                        "trust_adjustment":{"type":"trusted_local_ignored","factor":0.0},
                        "risk_after_trust":0.0,"mitre_boost":0.0,"asset_boost":0.0,"history_boost":0.0,
                        "pre_cap_risk":0.0,"capped":False,
                    },
                )
        ...
        # Historical recidivism boost — see module docstring for why this
        # (and only this) auxiliary signal is wired directly into the score.
        risk += history_boost
        ...
        return RiskResult(
            ...
            trust_score=trust,mitre_boost=mb,asset_boost=ab,history_boost=history_boost,
            response_action=resp,
            raw_components={
                "signature":S,"anomaly":A,"density":T,"drift_rate":B+R,
                "weights_used":{"signature":alpha,"anomaly":beta,"density":gamma,"drift_rate":delta},
                "base_risk":round(base_risk,2),
                "strength":strength,"strength_multiplier":strength_mult,
                ...
                "mitre_boost":round(mb,2),"asset_boost":round(ab,2),"history_boost":round(history_boost,2),
                "pre_cap_risk":round(pre_cap_risk,2),"capped":capped,
            },
        )
