...
class EvaluationRunner:
    ...
    def _score_dataset(
        ...
        """Run detection/risk scoring over the dataset ONCE and return the
        raw per-row (y_true, y_score, latency, category) arrays plus perf
        counters. This is the expensive part -- every threshold's metrics
        can be recomputed from these same arrays for free via
        MetricsCalculator.compute(threshold=...), so a sweep across N
        thresholds costs one pass over the data, not N passes.
        """
        ...
        if evaluation_failures:
            logger.warning(
                "[%s] Scoring completed with %d detection/risk failures "
                "out of %d rows.",
                name,
                evaluation_failures,
                len(y_true),
    ...
    def _report_from_scored(self, scored: dict, threshold: float):
        report = self._calc.compute(
            y_true=scored["y_true"],
            y_score=scored["y_score"],
            latencies_ms=scored["latencies"],
            peak_memory_mb=scored["peak_mem_mb"],
            mean_cpu_pct=scored["mean_cpu"],
            dataset_name=scored["name"],
            threshold=threshold,
            attack_categories=scored["categories"],
        )
        try:
            report.evaluation_failures = scored["evaluation_failures"]
            report.samples_evaluated = len(scored["y_true"])
        except Exception:
            pass
    ...
    def run(
        ...
        """Evaluate at a single threshold (backward-compatible entry point)."""
        scored = self._score_dataset(
            dataset_path, limit=limit, dataset_name=dataset_name, sample=sample,
        )
        return self._report_from_scored(scored, threshold)
    ...
    def run_sweep(
        ...
        """Evaluate the SAME scored dataset against multiple thresholds in
        one pass. Detection/risk scoring runs exactly once regardless of how
        many thresholds are swept -- only the cheap confusion-matrix bucketing
        in MetricsCalculator.compute() repeats per threshold.

        Returns a dict: {"dataset": name, "sweep": [report_dict, ...],
        "best_by_f1": report_dict_or_None}.
        """
        scored = self._score_dataset(
            dataset_path, limit=limit, dataset_name=dataset_name, sample=sample,
        )
        ...
        sweep_reports = []
        for t in thresholds:
            report = self._report_from_scored(scored, float(t))
            sweep_reports.append(report.to_dict())

        best = max(sweep_reports, key=lambda r: r["classification"]["f1"], default=None)
...
def _cli():
    ...
    parser.add_argument(
        ...
        default=None,
        help=(
            "Evaluate at multiple thresholds in one pass instead of one "
            "--threshold value. Accepts a comma list ('5,10,15,20') or a "
            "start:end:step range ('5:20:1'). Detection/risk scoring runs "
            "once; only per-threshold metrics are recomputed, so this is "
            "far cheaper than re-running --threshold N times. Overrides "
    ...
    def _parse_thresholds(spec: str) -> list:
        ...
        if ":" in spec:
            ...
            out, v = [], start
            # Use round() to sidestep float accumulation (5.0,5.1,5.2,...)
    ...
    thresholds = _parse_thresholds(args.threshold_sweep) if args.threshold_sweep else None
    ...
    # -----------------------------------------------------------------------
        ...
        print("\n" + "=" * 60)
        print(f"[{result['dataset']}] swept {len(thresholds)} thresholds: {thresholds}")
        if result["best_by_f1"]:
            b = result["best_by_f1"]
            print(f"Best F1 @ threshold={b['threshold_used']}: "
                  f"F1={b['classification']['f1']} "
                  f"P={b['classification']['precision']} "
                  f"R={b['classification']['recall']} "
                  f"FPR={b['classification']['false_positive_rate']}")
        print("=" * 60)
