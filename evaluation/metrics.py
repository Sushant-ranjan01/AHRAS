...
class MetricsReport:
    ...
    recall:             float = 0.0
    f1:                 float = 0.0
    auc:                Optional[float] = None   # None = undefined (only one class present), not 0
    ...
    roc_tpr:            Optional[List[float]] = field(default_factory=list)  # None = undefined
    ...
    def to_dict(self) -> dict:
        return {
            ...
            "classification": {
                ...
                "f1":                  round(self.f1, 4),
                # None (not NaN/0) when only one class is present in this file --
                # e.g. a zero-attack day has no positive class, so AUC is
                # mathematically undefined, not "0.0" (which would misleadingly
                # read as "the model is worthless") or "NaN" (which is not
                # valid JSON and breaks strict downstream parsers).
            ...
            "confusion_matrix": {
                ...
                # Same four numbers, human-readable: how many real attacks got
                # caught vs slipped through, and how many benign flows got
                # falsely flagged vs correctly left alone.
                "detected":          self.true_positives,   # attacks correctly flagged
                "missed":            self.false_negatives,  # attacks that slipped through
                "false_alarms":      self.false_positives,  # benign flows incorrectly flagged
                "correctly_cleared": self.true_negatives,   # benign flows correctly left alone
            },
            "performance": {
            ...
            "roc": {
                "fpr": self.roc_fpr,
                "tpr": self.roc_tpr,   # None when undefined -- see auc note above
            },
...
class MetricsCalculator:
    ...
    def compute(
        ...
        if attack_categories:
            ...
            cat_true_label: Dict[str, int] = {}
            for yt, yp, cat in zip(y_true, y_pred, attack_categories):
                c = (cat or "").strip() or "Unknown"
                if c not in cats:
                ...
                cats[c][key] += 1
                cats[c]["total"] += 1
                # A category's ground-truth label is homogeneous (e.g. every
                # "BENIGN" row is yt==0, every "DoS Hulk" row is yt==1), so
                # last-seen is safe here and tells us which stat set applies.
                cat_true_label[c] = yt
            for cat, cm in cats.items():
                _tp, _fp, _tn, _fn, total = cm["tp"], cm["fp"], cm["tn"], cm["fn"], cm["total"]
                if cat_true_label.get(cat) == 0:
                    # BUG FIX: this is a negative/benign-traffic category, not
                    # an attack type. Precision/recall/F1 are structurally
                    # undefined here (tp and fn are always 0 by construction,
                    # since yt==1 never happens for benign rows), which used
                    # to make BENIGN silently print "count: 0" and all-zero
                    # metrics -- reading as "0 samples classified" instead of
                    # what it actually was: 4200 real benign samples, most of
                    # them correctly left alone. Report specificity/FPR and
                    # the true sample count instead.
                    _spec = _tn / (_tn + _fp) if (_tn + _fp) else 0.0
                    per_class[cat] = {
                        "type": "benign",
                        "specificity": round(_spec, 3),
                        "false_positive_rate": round(1 - _spec, 3) if (_tn + _fp) else 0.0,
                        "count": total,
                        "tp": _tp, "fp": _fp, "tn": _tn, "fn": _fn,
                        "correctly_cleared": _tn,  # benign, correctly left alone
                        "false_alarms": _fp,       # benign, incorrectly flagged
                    }
                else:
                    ...
                    per_class[cat] = {
                        ...
                        "f1": round(_f, 3), "count": total,
                        "tp": _tp, "fp": _fp, "tn": _tn, "fn": _fn,
                        "detected": _tp,  # attack, correctly flagged
                        "missed": _fn,    # attack, slipped through
                    }
    ...
    def _roc_auc(self, y_true, y_score):
        # AUC/TPR are mathematically undefined when the sample only contains
        # one class (e.g. a zero-attack day has no positive class, so
        # "true positive rate" has a 0/0 denominator). sklearn's roc_curve
        # doesn't error on this -- it silently returns NaN throughout, which
        # then serializes as a literal `NaN` token (not valid JSON per the
        # spec) and reads as "the model failed" rather than "not applicable".
        # BUG FIX: detect the degenerate case up front and report it as
        # explicitly undefined (None) instead of computing/propagating NaN.
        if len(set(y_true)) < 2:
            logger.info("AUC/ROC undefined: only one class present in y_true")
            return [], None, None
        try:
