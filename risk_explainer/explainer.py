...
class RiskExplainer:
    ...
    def explain(self, risk_result: Any, uba_score: float = 0.0,
        ...
        # Fallback ONLY when trust is genuinely low (known-bad-ish), not
        # merely "unresolved/unknown domain". trust_score of ~0.1 means
        # the IP/domain couldn't be resolved at all (suspicious); trust
        # of 0.4-0.8 means a resolvable but non-allowlisted domain --
        # that is NOT threat intel evidence and must not be reported as
        # if it were a TI feed match.
        if ti_raw == 0.0:
            trust = float(getattr(r, "trust_score", 0.1))
            if trust <= 0.15:
                ti_raw = 0.5   # unresolved/suspicious, moderate signal only
            else:
                ti_raw = 0.0   # resolvable domain, no actual TI evidence
        ti_contrib = ti_raw * WEIGHTS["threat_intel"]
        ti_reason = (
            "Known malicious indicator in threat intel feeds" if ti_raw > 0.7
            else "Unresolved/suspicious source (no DNS PTR record)" if ti_raw > 0.3
        ...
        # ── 5. ML Anomaly / Signature component ────────────────────────────
        ...
        # Use the FULL signature confidence, not halved -- a strong
        # signature match (e.g. signature_score=90) is independently
        # significant evidence and should not be capped at 0.5 just
        # because it's blended with the anomaly flag.
        ml_raw = max(anom_raw, sig_factor)
        ...
        ml_reason = (
            ...
            else "Strong attack-pattern signature match" if sig_factor > 0.5
