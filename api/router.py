...
# ── ML OPERATIONS — PHASE 6 ─────────────────────────────────────────────────
class MLFeedback(BaseModel):
    alert_id: str
    label: int = Field(..., ge=0, le=1, description="1=true positive, 0=false positive")
    reason: Optional[str] = None
    prediction: Optional[float] = None
    model_version: Optional[str] = None
    features: dict = Field(default_factory=dict)

@api_router.get("/ml/status", summary="ML model lifecycle status")
async def ml_status(current_user: dict = Depends(require_permission(Perm.ML_READ))):
    from mlops import ModelRegistry, FeedbackStore
    r=ModelRegistry(); f=FeedbackStore()
    return {"registry":r.metadata(),"feedback":f.summary()}

@api_router.get("/ml/drift", summary="Latest model feature-drift report")
async def ml_drift(current_user: dict = Depends(require_permission(Perm.ML_READ))):
    from mlops import DriftMonitor
    return {"latest": DriftMonitor().latest()}

@api_router.get("/ml/models", summary="List registered ML models")
async def ml_models(current_user: dict = Depends(require_permission(Perm.ML_READ))):
    from mlops import ModelRegistry
    return {"models":ModelRegistry().list_models()}

@api_router.post("/ml/feedback", summary="Record analyst ML feedback")
async def ml_feedback(req: MLFeedback, current_user: dict = Depends(require_permission(Perm.ML_FEEDBACK))):
    from mlops import FeedbackStore
    row=FeedbackStore().add(alert_id=req.alert_id,label=req.label,model_version=req.model_version,prediction=req.prediction,features=req.features,analyst=current_user.get("sub"),reason=req.reason)
    # Also feed existing adaptive risk learner when a cached risk vector exists.
    try:
        import main as m
        cached=getattr(m,"_risk_feedback_cache",{}).get(req.alert_id) or {}
        comps=cached.get("components") or req.features
        if comps and hasattr(m,"weight_learner"):
            from adaptive_learning.weight_learner import FeedbackSample
            src=cached.get("src_ip", "")
            risk=float(cached.get("risk_score", req.prediction or 50))
            m.weight_learner.record_feedback(FeedbackSample(src_ip=src,label=req.label,components=comps,predicted_risk=risk))
    except Exception as e:
        logger.warning("Adaptive feedback bridge skipped: %s", e)
    audit_logger.info("user=%s action=ml_feedback alert=%s label=%s",current_user.get("sub"),req.alert_id,req.label)
    return {"recorded":True,"feedback":row}

@api_router.post("/ml/register", summary="Register current model artifact")
async def ml_register(version: str, current_user: dict = Depends(require_permission(Perm.ML_PROMOTE))):
    from mlops import ModelRegistry
    entry=ModelRegistry().register(version, metadata={"registered_by":current_user.get("sub"),"framework":"scikit-learn"})
    audit_logger.info("user=%s action=ml_register version=%s",current_user.get("sub"),version)
    return entry

@api_router.post("/ml/promote/{version}", summary="Promote a registered model")
async def ml_promote(version: str, current_user: dict = Depends(require_permission(Perm.ML_PROMOTE))):
    from mlops import ModelRegistry
    try: entry=ModelRegistry().promote(version)
    except KeyError: raise HTTPException(404,"Model version not found")
    audit_logger.info("user=%s action=ml_promote version=%s",current_user.get("sub"),version)
    return {"promoted":True,"model":entry}
