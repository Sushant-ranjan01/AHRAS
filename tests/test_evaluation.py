...
class TestFeatureToDetectionMapping:
    """Regression tests for evaluation.runner._record_to_flow.
    ...
    NOTE: this used to test evaluation.runner._feature_to_detection, which
    read attack_type off record.attack_category (ground truth) -- a
    leakage path removed in the evaluation-pipeline fix. The port-count
    derivation logic these tests guard moved unchanged into
    _record_to_flow(), which now feeds the real HybridDetectionEngine
    instead of shortcutting straight to a dataset-label lookup.
    """

    def test_destination_port_number_not_used_as_unique_ports_count(self):
        from evaluation.dataset_loader import DatasetRecord
        from evaluation.runner import _record_to_flow
        ...
        assert flow.unique_ports != 443
        assert flow.unique_ports == 0
...
class TestCicidsLabelEncodingFix:
    ...
    the label is only used for ground-truth reporting (attack-category
    breakdowns in MetricsCalculator), never for prediction -- but it must
    not leak the replacement character into report output.
    """
