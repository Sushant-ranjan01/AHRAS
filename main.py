...
from security import RateLimiter, SecurityHeadersMiddleware, audit_event
from observability import (MetricsMiddleware, generate_latest, CONTENT_TYPE_LATEST,
    ACTIVE_SESSIONS, LOGIN_FAILURES, WORKER_ERRORS, EVENTS, WORKER_HEALTH,
    QUEUE_DEPTH, PROCESS_CPU, PROCESS_MEMORY, MONGODB_UP,
    register_worker, heartbeat, worker_error, snapshot, configure_json_logging)
...
async def lifespan(app):
    # startup
    register_worker("analysis")
    register_worker("packet_sniffer")
    register_worker("flow_generator")
    sniffer.start(); heartbeat("packet_sniffer")
    flow_gen.start(); heartbeat("flow_generator")
    threading.Thread(target=_analysis_worker, name="ahras-analysis-worker", daemon=True).start()
...
_api_limiter = RateLimiter(config.RATE_LIMIT_PER_MINUTE, 60)
_login_limiter = RateLimiter(config.LOGIN_RATE_LIMIT_PER_MINUTE, 60)

@app.middleware("http")
async def rate_limit_middleware(request: Request, call_next):
    ip = request.client.host if request.client else "unknown"
    limiter = _login_limiter if request.url.path == "/api/auth/login" else _api_limiter
    allowed, retry = limiter.allow(ip)
    if not allowed:
        return Response(content=json.dumps({"detail": "Rate limit exceeded", "retry_after": retry}), status_code=429, media_type="application/json", headers={"Retry-After": str(retry)})
...
async def login(request: Request, form: OAuth2PasswordRequestForm = Depends()):
    ...
    if not token:
        LOGIN_FAILURES.inc()
        audit_event("login_failed", form.username, ip, outcome="failure")
        raise HTTPException(401, "Invalid credentials")
    user = auth_mgr.get_user(form.username)
    refresh = auth_mgr.issue_refresh_for(form.username)
    audit_event("login_success", form.username, ip, resource="auth")
    uba_engine.record_login(form.username, ip)
    ACTIVE_SESSIONS.set(len(auth_mgr._active_sessions))
...
async def refresh_token(req: RefreshTokenReq):
    result = auth_mgr.refresh(req.refresh_token)
    if not result:
        raise HTTPException(401, "Invalid or expired refresh token")
    access, refresh = result
    audit_event("token_refresh", "system", resource="auth")
...
async def change_password(req: ChangePasswordReq, cu: dict = Depends(get_current_user)):
    ...
    ok, reason = validate_password(req.new_password, cu.get("sub", ""))
    if not ok:
...
async def logout(cu: dict = Depends(get_current_user)):
    auth_mgr.logout(cu.get("session_id","")); ACTIVE_SESSIONS.set(len(auth_mgr._active_sessions)); audit_event("logout", cu.get("sub",""), resource="auth"); return {"message": "Logged out"}
...
# ── SOC Analyst Command Center (Phase 5) ─────────────────────────────────────
@app.get("/api/soc/overview")
async def soc_overview(_: dict = Depends(get_current_user)):
    """Compact analyst queue for triage: alerts, incidents, cases and notifications."""
    alerts = alert_mgr.list_alerts(limit=200)
    cases = case_mgr.list_cases(limit=200)
    incidents = corr_engine.recent_incidents(100)
    notifs = alert_mgr.get_notifications(unread_only=True)
    severity = {k: 0 for k in ("LOW", "MEDIUM", "HIGH", "CRITICAL")}
    open_alerts = 0
    for a in alerts:
        severity[a.get("priority", "LOW")] = severity.get(a.get("priority", "LOW"), 0) + 1
        if a.get("status") not in ("RESOLVED",): open_alerts += 1
    case_counts = {}
    for c in cases:
    ...
    return {
        "queue": {
            ...
            "open_cases": case_counts.get("OPEN", 0) + case_counts.get("ASSIGNED", 0),
            "unacknowledged_incidents": sum(1 for i in incidents if not i.get("acknowledged")),
...
@app.get("/api/soc/alerts/{alert_id}/context")
async def soc_alert_context(alert_id: str, _: dict = Depends(get_current_user)):
    """Build one analyst investigation view around an alert."""
    alert = next((a for a in alert_mgr.list_alerts(limit=2000) if a.get("alert_id") == alert_id), None)
    if not alert:
        raise HTTPException(404, "Alert not found")
    ip = alert.get("target_ip") or ""
    events = evt_mgr.recent_events(1000)
    related_events = [e for e in events if ip and (e.get("src_ip") == ip or e.get("dst_ip") == ip)]
    related_events = related_events[:100]
    cases = case_mgr.list_cases(limit=200)
    related_cases = [c for c in cases if alert_id in (c.get("alert_ids") or []) or ip in (c.get("affected_ips") or [])]
    intel = None
    if ip:
        try:
            intel = threat_intel.check_ip(ip)
            if hasattr(intel, "to_dict"): intel = intel.to_dict()
        except Exception as exc:
            logger.debug("SOC TI lookup unavailable for %s: %s", ip, exc)
    mitre = mitre_map.map(alert.get("attack_type", ""))
    mitre_data = None
    if mitre:
        mitre_data = {"technique_id": mitre.technique_id, "technique": mitre.technique_name, "tactic": mitre.tactic}
    return {
        "alert": alert,
...
@app.get("/api/soc/timeline/{ip}")
async def soc_timeline(ip: str, limit: int = Query(200, ge=1, le=1000), _: dict = Depends(get_current_user)):
    events = evt_mgr.recent_events(2000)
    rows = [e for e in events if e.get("src_ip") == ip or e.get("dst_ip") == ip]
    return {"ip": ip, "count": len(rows), "events": rows[:limit]}

@app.get("/api/soc/export/alerts.csv")
async def soc_export_alerts(_: dict = Depends(get_current_user)):
    import csv, io
    rows = alert_mgr.list_alerts(limit=2000)
    buf = io.StringIO()
    writer = csv.DictWriter(buf, fieldnames=["alert_id","title","priority","status","submitted_by","target_ip","attack_type","created_at","updated_at"])
    writer.writeheader()
    for row in rows:
        writer.writerow({k: row.get(k, "") for k in writer.fieldnames})
    return Response(content=buf.getvalue(), media_type="text/csv", headers={"Content-Disposition": f'attachment; filename="ahras_alerts_{int(time.time())}.csv"'})
...
@app.get("/health/live")
async def liveness():
    return {"status": "alive", "version": "7.0.0", "timestamp": datetime.now(timezone.utc).isoformat()}

@app.get("/health/ready")
async def readiness():
    workers = snapshot()
    mongo = db_health()
    worker_ok = all(v.get("healthy") for v in workers.values()) if workers else False
    mongo_ok = mongo.get("status") == "connected"
    status = "ready" if worker_ok and mongo_ok else "not_ready"
    return Response(content=json.dumps({"status": status, "mongodb": mongo, "workers": workers}, default=str), status_code=200 if status == "ready" else 503, media_type="application/json")
...
async def health():
    ...
    try:
        ...
        pass
    MONGODB_UP.set(1 if mongo.get("status") == "connected" else 0)
    for name, state in workers.items():
        WORKER_HEALTH.labels(name).set(1 if state.get("healthy") else 0)
    QUEUE_DEPTH.labels("packets").set(_pkt_q.qsize())
    QUEUE_DEPTH.labels("flows").set(_flow_q.qsize())
    return {
        ...
        "mongodb": mongo,
        "workers": workers,
        "queues": {"packets": _pkt_q.qsize(), "flows": _flow_q.qsize()},
        "modules": {
