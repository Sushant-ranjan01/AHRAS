...
from utils.llm_client import call_local_llm, is_llm_configured, model_name
...
class SecurityIntelligenceEngine:
    ...
    The evidence chain (failed_logins, historical_incidents, MITRE mapping, etc.)
    is always built deterministically from real AHRAS subsystem data — that part
    never depends on an LLM and is what tests/automation should assert against.
    On top of that grounded evidence, if a local model is configured
    (AHRAS_LOCAL_LLM_URL, see utils/llm_client.py / README "Enabling Ollama"),
    each explainer also asks it to turn the evidence into a natural-language
    write-up, returned under "llm_narrative". If no LLM is configured or it's
    unreachable, "llm_narrative" is simply omitted/None and "generated_by" is
    "template" — the deterministic "conclusion"/"incident_summary" fields are
    always present either way, so callers never have to special-case this.
    ...
    - Optional local-LLM narration layer (Ollama-compatible) on top of the
      grounded evidence chain, never in place of it
    """

    def _llm_narrate(self, instruction: str, grounded_facts: dict) -> Dict[str, Optional[str]]:
        """
        Ask the configured local LLM to narrate an already-computed, grounded
        result. The LLM is only ever given facts AHRAS has already derived
        deterministically (grounded_facts) — it narrates, it does not decide
        risk, severity, or evidence. Returns {"llm_narrative": str|None,
        "generated_by": "local-llm:<model>"|"template"}.
        """
        if not is_llm_configured():
            return {"llm_narrative": None, "generated_by": "template"}
        prompt = (
            f"{instruction}\n\n"
            "Base your answer only on these facts (do not invent evidence, IPs, "
            "IDs, or numbers that are not listed here):\n"
            f"{json.dumps(grounded_facts, default=str)}\n"
        )
        raw = call_local_llm(prompt)
        if not raw:
            return {"llm_narrative": None, "generated_by": "template"}
        return {"llm_narrative": raw, "generated_by": f"local-llm:{model_name()}"}
    ...
    def alert_explainer(self, alert_name: str, risk_score: float, evidence: dict) -> dict:
        ...
        # Real event/alert IDs only — pulled from the live alert_history AHRAS
        # actually recorded (see build_live_alert_evidence). Never fabricated;
        # an empty list here honestly means "no linked events available" rather
        # than making up placeholder IDs.
        raw_event_ids = []
        for a in (evidence.get("alert_history") or [])[:10]:
            if isinstance(a, dict):
                eid = a.get("id") or a.get("alert_id") or a.get("_id")
                if eid:
                    raw_event_ids.append(str(eid))
        ...
        result.update(self._llm_narrate(
            "You are a SOC analyst assistant. Rewrite the deterministic "
            "conclusion below as a clear, analyst-facing explanation of why "
            "this alert's risk score is what it is. Do not add any evidence "
            "not present in the facts.",
            {"alert_name": alert_name, "risk_score": risk_score,
             "conclusion": conclusion, "evidence": evidence_chain},
        ))
        return result
    ...
    def investigate_incident(self, src_ip: str, context: dict) -> dict:
        ...
        result.update(self._llm_narrate(
            "You are a SOC analyst assistant writing an incident investigation "
            "summary for another analyst. Turn the structured facts below into "
            "a short prose write-up (what happened, likely attack path, blast "
            "radius, recommended next step). Do not invent IPs, users, hosts, "
            "or MITRE techniques not present in the facts.",
            incident_summary,
        ))
        return result

    def plain_english_summary(self, overview: dict) -> dict:
        """
        Turns the SOC's current state into a summary a non-security person can
        read and act on: what's happening, why it matters, and what (if
        anything) they need to do. Always returns a deterministic digest;
        layers an LLM-narrated version on top via self._llm_narrate() if a
        local model is configured.
        """
        total_alerts = int(overview.get("total_alerts", 0) or 0)
        critical = int(overview.get("critical_alerts", 0) or 0)
        high = int(overview.get("high_alerts", 0) or 0)
        current_risk = float(overview.get("current_risk", 0) or 0)
        top_sources = overview.get("top_threat_sources", []) or []
        blocked = int(overview.get("blocked_ips", 0) or 0)

        if total_alerts == 0 and current_risk < 3:
            whats_happening = "Nothing concerning right now — your network traffic looks normal."
            why = ("No unusual login attempts, scans, or malicious IP activity has been "
                   "detected in the current monitoring window.")
            what_to_do = "No action needed. AHRAS is actively watching and will alert you the moment something changes."
        else:
            severity_word = "a serious" if critical > 0 else "a moderate" if high > 0 else "a low-level"
            whats_happening = (
                f"AHRAS has flagged {total_alerts} security alert{'s' if total_alerts != 1 else ''} "
                f"and currently rates overall risk at {current_risk:.1f}/10 — {severity_word} level of concern."
            )
            reasons = []
            if critical:
                reasons.append(f"{critical} of the alerts are CRITICAL severity")
            if high:
                reasons.append(f"{high} are HIGH severity")
            if top_sources:
                reasons.append(f"most of the activity is coming from {top_sources[0]}")
            why = ("This is because " + ", and ".join(reasons) + ".") if reasons else (
                "The system detected traffic patterns that don't match normal behavior."
            )
            if blocked:
                what_to_do = (
                    f"AHRAS has already automatically blocked {blocked} suspicious source(s) — "
                    "no action needed unless you want a security analyst to review the report."
                )
            else:
                what_to_do = (
                    "Nothing has been blocked automatically yet. It's worth having a security "
                    "analyst take a look, or turning on auto-block in Settings."
                )
        ...
        plain = {
            ...
            "risk_in_plain_words": self._risk_word(current_risk),
        }

        result = {"summary": plain}
        result.update(self._llm_narrate(
            "You are explaining a cybersecurity dashboard to someone with NO "
            "technical background (e.g. a business owner or HR manager). Rewrite "
            "the facts below as 3-4 short plain-English sentences: what's "
            "happening, why it matters, and whether they personally need to do "
            "anything. Avoid jargon (no 'IOC', 'CVE', 'lateral movement', etc — "
            "explain any unavoidable term in plain words). Do not invent numbers "
            "or events that aren't in the facts.",
            {**plain, "total_alerts": total_alerts, "critical_alerts": critical,
             "high_alerts": high, "current_risk": current_risk, "blocked_ips": blocked,
             "top_threat_sources": top_sources[:3]},
        ))
        return result
    ...
    def _risk_word(score: float) -> str:
        if score < 3:
            return "Low — everything looks normal"
        if score < 6:
            return "Moderate — worth a look, nothing urgent"
        if score < 8.5:
            return "High — should be reviewed soon"
