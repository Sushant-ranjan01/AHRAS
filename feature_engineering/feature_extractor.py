...
# NOTE: "unique_ports" was removed from the ML feature vector (25-08-27).
# CICIDS/CSE-CIC-IDS2018 is labeled per-flow, so cicids_row_to_raw_features()
# could only ever emit a constant 1.0 for it (see the comment that used to be
# above the old return dict). Training on a constant while runtime
# (FlowFeatureExtractor) fed in real observed values (2, 5, 10, 30+...) meant
# the model saw a feature at inference time whose training distribution had
# zero variance -- a hard train/runtime distribution mismatch, compounded by
# the risk engine's own unique_ports>10 / unique_ports>30 rules double-
# counting the same signal (see risk_scorer.py rule_boosts). unique_ports is
# still tracked and used there as a real-time rule-based signal; it is simply
# no longer part of what the anomaly model is asked to learn/score, since
# CICIDS gives it no genuine distribution to learn from.
FEATURE_NAMES = [
    "packet_count", "byte_count", "avg_packet_size", "max_packet_size",
...
def cicids_row_to_raw_features(row: Mapping[str, Any]) -> dict[str, float]:
    ...
    # unique_ports intentionally omitted -- see FEATURE_NAMES comment above.
    return {
...
class FlowFeatureExtractor:
    def extract(self, flow: FlowRecord) -> FeatureVector:
        ...
        # NOTE: flow.unique_ports is still computed by the sensor/flow layer
        # and is still used directly by RiskEngine's rule-based boosts
        # (unique_ports>10 / >30). It is deliberately NOT fed to the ML
        # model here -- see FEATURE_NAMES comment in this module.
        raw = {
